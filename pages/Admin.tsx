              ...
              {(isAdding === 'category' || editingItem?.type === 'category') && (
                <form onSubmit={handleCategorySubmit} className="bg-white rounded-2xl border border-gray-200 p-8 shadow-xl animate-in slide-in-from-top-4 duration-500 space-y-6">
                  <div className="grid grid-cols-2 gap-8">
                    ...
                    <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-1">Parent Category</label><select value={catForm.parentId || ''} onChange={e => setCatForm({ ...catForm, parentId: e.target.value || null })} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-5 py-3.5 text-sm font-bold"><option value="">None (Top Level)</option>{hierarchicalCategories.map(c => <option key={c.id} value={c.id}>{'\u00A0'.repeat(c.level * 4)}{c.name}</option>)}</select></div>
                  </div>
